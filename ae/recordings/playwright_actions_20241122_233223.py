...
# Recording started at: 2024-11-22 23:32:23
...
async def run_recorded_actions(page: Page):
    try:
        # Start of recorded actions
            ...
            time.sleep(0.5)  # Small delay between actions
            page.eval_on_selector("[mmid='2853']", """e => { let originalBorderStyle = e.style.border; e.classList.add("agente-ui-automation-highlight"); e.addEventListener("animationend", () => { e.classList.remove("agente-ui-automation-highlight") });}""")
            time.sleep(0.5)  # Small delay between actions
